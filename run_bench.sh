...
srun_output=$(srun --partition=$mactype --time=00:10:00 perf stat -e task-clock -r 3 ./$studentexec $testfile $numthreads 2>&1)
...
for bench in ${BENCHES[@]}; do
    if [[ "$bench" == "bench-70.perf" ]]; then
        echo "Skipping $bench as per condition"
        continue
    fi
    if [[ "$bench" == "bench-71.perf" ]]; then
        echo "Skipping $bench as per condition"
        continue
    fi
    if [[ "$bench" == "bench-72.perf" ]]; then
        echo "Skipping $bench as per condition"
        continue
    fi
